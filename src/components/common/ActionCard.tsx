...
const colorMap = (color: string) => {
    switch (color) {
        case 'primary': return { bg: 'bg-primary', bgOpacity: 'bg-primary/10', borderOpacity: 'border-primary/20', hover: 'hover:bg-primary/90', iconColor: 'text-primary' };
        case 'success': return { bg: 'bg-success', bgOpacity: 'bg-success/10', borderOpacity: 'border-success/20', hover: 'hover:bg-success/90', iconColor: 'text-success' };
        case 'warning': return { bg: 'bg-warning', bgOpacity: 'bg-warning/10', borderOpacity: 'border-warning/20', hover: 'hover:bg-warning/90', iconColor: 'text-warning' };
        case 'danger': return { bg: 'bg-danger', bgOpacity: 'bg-danger/10', borderOpacity: 'border-danger/20', hover: 'hover:bg-danger/90', iconColor: 'text-danger' };
        default: return { bg: 'bg-gray-500', bgOpacity: 'bg-gray-500/10', borderOpacity: 'border-gray-500/20', hover: 'hover:bg-gray-500/90', iconColor: 'text-gray-500' };
    }
}
...
const ActionCard: React.FC<ActionCardProps> = ({ title, description, icon, color, onClick, buttonText, disabled }) => {
    ...
    return (
        ...
        >
            ...
            {/* FIXED: Icon with proper visibility in both light and dark modes */}
            <div className={`absolute top-4 right-4 ${colors.iconColor} opacity-80 hover:opacity-100 transition-opacity`}>
                {icon}
            </div>
